...
func NanoTimeBeforeNowSeconds(secs int) int {
    now := time.Now().UnixNano()
    now = now - int64((secs *1e9))
    return int(now)
}
...
const letterRunesNum = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"
func RandAlphaNumbericString(size int) string {
    b := make([]byte, size)
    for i := range b {
