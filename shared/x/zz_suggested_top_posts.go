...
) // (shortname .TableNameGo "err" "res" "sqlstr" "db" "XOLog") -}}//(schema .Schema .Table.TableName) -}}// .TableNameGo}}// SuggestedTopPost represents a row from 'sun.suggested_top_posts'.
...
func (stp *SuggestedTopPost) Insert(db XODB) error {
	...
	const sqlstr = `INSERT INTO sun.suggested_top_posts (` +
		`PostId` +
		`) VALUES (` +
		`?` +
		`)`
...
func (stp *SuggestedTopPost) Replace(db XODB) error {
	...
	const sqlstr = `REPLACE INTO sun.suggested_top_posts (` +
		`PostId` +
		`) VALUES (` +
		`?` +
		`)`
...
func (stp *SuggestedTopPost) Update(db XODB) error {
	...
	// sql query
	const sqlstr = `UPDATE sun.suggested_top_posts SET ` +
		`PostId = ?` +
		` WHERE Id = ?`
...
func (u *__SuggestedTopPost_Selector) _stoSql() (string, []interface{}) {
	...
	sqlstr := "SELECT " + u.selectCol + " FROM sun.suggested_top_posts"
...
func (u *__SuggestedTopPost_Updater) Update(db XODB) (int, error) {
	...
	sqlstr := `UPDATE sun.suggested_top_posts SET ` + sqlUpdate
...
func MassInsert_SuggestedTopPost(rows []SuggestedTopPost, db XODB) error {
	...
	// sql query
	sqlstr := "INSERT INTO sun.suggested_top_posts (" +
		"PostId" +
		") VALUES " + insVals
...
func MassReplace_SuggestedTopPost(rows []SuggestedTopPost, db XODB) error {
	...
	insVals := insVals_[0 : len(insVals_)-1]
	// sql query
	sqlstr := "REPLACE INTO sun.suggested_top_posts (" +
		"PostId" +
		") VALUES " + insVals
